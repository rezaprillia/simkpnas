...
import { PositionType, KategoriAktif } from './positiontype.model';
...
@Injectable()
export class PositionTypeService {
  private resourceUrlVisi = this.a.SERVER_URL + '/master/PosTypes';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlVisi + '/retrieve?token=' + token + '&idpost=' + "'" + id + "'")
  }
  ...
  save(data: PositionType): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: PositionType): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlVisi + '/delete?token=' + token + '&idpost=' + data.idpost)
  }
