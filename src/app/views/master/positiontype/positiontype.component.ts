...
import {Search} from './positiontype.model';
...
export class PositionTypeComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(PositionTypeService) private roleService: PositionTypeService
  ) {
    this.search = {
      idpost: '',
      position_type: '',
  ...
  searching() {
    this.roleService.getByName(this.search.idpost).subscribe(resp => {
      this.roleService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.idpost !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.idpost).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
