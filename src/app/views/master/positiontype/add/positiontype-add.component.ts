...
// @Component({
//   selector: 'app-add-positiontype',
//   templateUrl: './positiontype-add.component.html',
//   styleUrls: ['./positiontype-add.component.scss']
// })
// export class PositionTypeAddComponent implements OnInit {
...
import {Menu, PositionType, KategoriAktif} from '../positiontype.model';
...
import { DxiItemComponent } from 'devextreme-angular/ui/nested/item-dxi';
 ...
 export class PositionTypeAddComponent implements OnInit, AfterViewInit, AfterContentInit {
   @Input() isEdit;
   @Input() isDetail;
   @Input() editItem;
   @Input() addVisible;
   ...
   constructor(private roleService: PositionTypeService) {
   ...
   ngOnInit() {
    if (this.isEdit || this.isDetail) {
      ...
      this.roleService.getById(this.editItem).subscribe(respRole => {
        ...
        this.role = {
          idpost: respRole.d.idpost,
          position_type: respRole.d.position_type,
  ...
  checkBoxToggled(e) {
   ...
   onSaveConf() {
     ...
     this.role.position_type = this.role.position_type.toUpperCase();
     ...
     if (!this.isEdit) {
       ...
       this.roleService.save(this.role).subscribe(resp => {
         ...
         if (success) {
           ...
           notify({message: 'Position Type berhasil disimpan', position: {my: 'center top', at: 'center top'}},
            'success', 3000);
           this.hide();
         } else {
       ...
       this.roleService.update(this.role).subscribe(resp => {
        ...
        success = true;
         ...
         if (success) {
           this.options.message = 'Position Type updated';
           notify(this.options, 'success', 3000);
           this.hide();
         } else {
