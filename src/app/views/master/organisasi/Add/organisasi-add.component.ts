...
import {Menu, Role, KategoriAktif} from '../organisasi.model';
...
import { DxiItemComponent } from 'devextreme-angular/ui/nested/item-dxi';
 ...
 @Component({
   selector: 'app-add-organisasi',
   templateUrl: './organisasi-add.component.html',
   providers: []
 })
 export class AddOrganisasi implements OnInit, AfterViewInit, AfterContentInit {
   @Input() isEdit;
   @Input() isDetail;
   @Input() editItem;
   @Input() addVisible;
   ...
   constructor(private roleService: OrganisasiService) {
   ...
   ngOnInit() {
    if (this.isEdit || this.isDetail) {
      ...
      this.roleService.getById(this.editItem).subscribe(respRole => {
        ...
        if(respRole.d.mulai_berlaku=='') {respRole.d.mulai_berlaku=null};
        if(respRole.d.akhir_berlaku=='') {respRole.d.akhir_berlaku=null};
        this.role = {
          idorg: respRole.d.idorg,
          parent_idorg: respRole.d.parent_idorg,
          entitas_name: respRole.d.entitas_name,
          mulai_berlaku: respRole.d.mulai_berlaku,
          akhir_berlaku: respRole.d.akhir_berlaku,
          tahun: respRole.d.tahun,
          entitas_level: respRole.d.entitas_level,
  ...
  checkBoxToggled(e) {
   ...
   onSaveConf() {
     ...
     this.role.entitas_name = this.role.entitas_name.toUpperCase();
