...
//   styleUrls: ['./orgtype.component.scss']
// })
...
import {Search} from './orgtype.model';
...
export class OrgTypeComponent implements AfterViewInit {
  ...
  addNewButton() {
    ...
    const $customButton = $('<div id="addNewRole">').dxButton({
      icon: 'add',
      text: 'Tambah Tipe Organisasi Baru',
      onClick: function () {
        ini.isAdd = true;
  ...
  searching() {
    this.roleService.getByName(this.search.type_name).subscribe(resp => {
      this.roleService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.idorgt !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.idorgt).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
