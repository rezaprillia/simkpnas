...
import {Menu, Role, KategoriAktif} from '../position.model';
...
import { DxiItemComponent } from 'devextreme-angular/ui/nested/item-dxi';
 ...
 export class PositionAddComponent implements OnInit, AfterViewInit, AfterContentInit {
   @Input() isEdit;
   @Input() isDetail;
   @Input() editItem;
   @Input() addVisible;
   ...
   constructor(private roleService: PositionService) {
    ...
    if (this.isEdit || this.isDetail) {
      ...
      this.roleService.getById(this.editItem).subscribe(respRole => {
        ...
        this.role = {
          ...
          position_title: respRole.d.position_title,
          idpost: respRole.d.idpost,
          kode_sap: respRole.d.kode_sap,
          sebutan_sap: respRole.d.sebutan_sap,
          parent_idpos: respRole.d.parent_idpos,
        };
  ...
  checkBoxToggled(e) {
   ...
   onSaveConf() {
     ...
     this.role.position_title = this.role.position_title.toUpperCase();
     //console.log('isdisplayed sebelum disave = ' + this.role.position_title);
     //const nilai: number = this.role.position_title;
    //  this.role.isdisplayed = nilai.toString();
     let success = false;
     if (!this.isEdit) {
       this.role.idpos = null;
       this.roleService.save(this.role).subscribe(resp => {
         ...
         if (success) {
           ...
           notify({message: 'Posisi berhasil disimpan', position: {my: 'center top', at: 'center top'}},
            'success', 3000);
           this.hide();
         } else {
