...
import {Search} from './position.model';
...
export class PositionComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(PositionService) private roleService: PositionService
  ) {
    this.search = {
      idpos: '',
      position_title: '',
  ...
  searching() {
    this.roleService.getByName(this.search.idpos).subscribe(resp => {
      this.roleService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.idpos !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.idpos).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
